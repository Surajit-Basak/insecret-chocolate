...
export default async function HomePageSettings() {
    ...
    // Define the desired order of sections, excluding Hero which is handled separately.
    ...
    return (
        <div className="flex flex-col gap-4 py-4">
            ...
            {/* SEO Section First */}
            {(() => {
                 const seoKeys = homepageSections['SEO'];
                 const seoSettings = settings?.filter(s => seoKeys.includes(s.key));
                 if (!seoSettings || seoSettings.length === 0) return null;
                 return (
                    <Card>
                        <CardHeader>
                            <CardTitle>SEO</CardTitle>
                             <CardDescription>Manage the meta title and description for the homepage.</CardDescription>
                        </CardHeader>
                        <CardContent>
                            <SiteSettingsForm settings={seoSettings} revalidatePaths={['/']} />
                        </CardContent>
                    </Card>
                 )
            })()}
            ...
            {orderedSections.map((sectionTitle) => {
                const keys = homepageSections[sectionTitle as keyof typeof homepageSections];
                const sectionSettings = settings?.filter(s => keys.some(k => s.key.startsWith(k)));
                if (!sectionSettings || sectionSettings.length === 0) return null;
