...
interface SignatureCollectionFormProps {
  item?: SignatureCollection;
}

export default function SignatureCollectionForm({ item }: SignatureCollectionFormProps) {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      if (item) {
        ...
        // Create new item
        const { error } = await supabase.from('signature_collections').insert([data]);
        if (error) throw error;
        toast({ title: "Success", description: "Item created successfully." });
      }
      await revalidatePathsAction(['/']);
      router.push('/admin/pages/home/signature-collections');
      router.refresh();
    } catch (error: any) {
      toast({
        variant: "destructive",
  ...
  const handleDelete = async () => {
    ...
    try {
        const { error } = await supabase.from('signature_collections').delete().eq('id', item.id);
        if (error) throw error;
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="Luxury Bars" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Textarea placeholder="A short description of the collection item." {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>Where the "Shop Now" button should link to.</FormDescription>
              <FormMessage />
            </FormItem>
          )}
        />
